...
from .hybrid_brute_match import HybridBruteMatcher
...
def hybrid_nnf_analogy_loss(a, a_prime, b, b_prime, patch_size=3, patch_stride=1):
    '''image shapes: (channels, rows, cols)
    '''
    bs = b.shape
...
class NNFState(object):
    ...
    def update(self, x, num_steps=5):
        ...
        x_normed = x_patches #self.matcher.normalize_patches(x_patches)
        for i in range(num_steps):
            self.matcher.update_with_patches(x_normed)


class HybridNNFState(object):
    def __init__(self, matcher, f_layer):
        self.matcher = matcher
        mis = matcher.input_shape
        self.placeholder = K.placeholder(mis[::-1])
